"""
https://github.com/zifuwan/Sigma/blob/main/models/encoders/vmamba.py
"""
import os
import time
import math
...
class Backbone_VSSM(VSSM):
    ...
    def forward(self, x):
        ...
        # (B,C,H,W) -> (B, H/patch_size, W/patch_size, dim[0])
        x = self.patch_embed(x)
        outs = []
        # self.layers的列表长度为 len(self.depth)=4，前面每个
        for i, layer in enumerate(self.layers):
            # 0~2: (B,H/patch_size,W/patch_size,dim[i]) -> (B,H/patch_szie/2,W/patch_size/2,dim[i+1])
            # 3: (B,H/patch_size/2/2/2,W/patch_size/2/2/2,dim[3]) -> (B,H/patch_szie/2/2.2,W/patch_size/2/2/2,dim[3])
            # o: x下采样之前的特征，x：o经过下采样后的特征
...
class vmamba_tiny(nn.Module):
    def __init__(self):
        ...
        self.vmamba = Backbone_VSSM(
                        depths=[2, 2, 9, 2],
                        dims=96,
                        pretrained='/home/nk/lwy/Code/weights/vssmtiny_dp01_ckpt_epoch_292.pth',
                        mlp_ratio=0.0,
    ...
    def get_stage_channels():
        ...
        # torch.Size([1, 96, 88, 88])
        # torch.Size([1, 192, 44, 44])
        # torch.Size([1, 384, 22, 22])
        # torch.Size([1, 768, 11, 11])
...
if __name__ == '__main__':
    ...
    in_tensor = torch.randn(1, 3, 768, 768).cuda()
    model = model.cuda()
