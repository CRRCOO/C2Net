...
model = C2Net(channels=48, lle_channels=16)

# 定义numpy输入矩阵
bs = 1
test_img = np.random.random((bs, 3, 352, 352)).astype('float32')
test_dop = np.random.random((bs, 1, 352, 352)).astype('float32')

# 定义 pytorch & jittor 输入矩阵
pytorch_test_img = torch.Tensor(test_img).cuda()
test_dop = torch.Tensor(test_dop).cuda()
...
for i in range(10):
	...
	pytorch_result = pytorch_model(pytorch_test_img, test_dop)  # Pytorch热身
torch.cuda.synchronize()
sta = time.time()
for i in range(turns):
	# pytorch_result = pytorch_model(pytorch_test_img)
	pytorch_result = pytorch_model(pytorch_test_img, test_dop)
